...
who is speaking, then how code, docstrings, commits, PRs and issues are written.
**What** the project is lives in [docs/](docs/README.md); **which command to
run** lives in [CONTRIBUTING.md](CONTRIBUTING.md). Every rule below is here
because its absence cost a wrong number, a lost diff, or a round trip.

## AI-assisted contributions

Agents author most of what lands here, and that is the intent. What must never
blur is **who is speaking**. Code is checked — by tests, gates and an oracle — so
its authorship is a footnote. A discussion is *trusted*: nobody can re-run a
sentence to find out whether it is true, and a reader who cannot tell a
maintainer's judgement from a model's paragraph has lost the only thing a
discussion carries. Same three rules as
[linopy's](https://github.com/PyPSA/linopy/blob/master/AGENTS.md), so anyone
moving between the two projects reads one policy.

**1. Mark what an agent generated**, at the top of the generated section — a
footer at the bottom does not count, because the reader has already read it as
the human's by then.

```markdown
Why this matters to me — one or two lines, the human's own.
...
Verbose evidence — benchmark tables, logs, long reproductions — goes inside the
`<details>` so it cannot drown the discussion. Never mix hand-written and
generated prose silently. The same marker belongs on an agent-filed issue and on
any comment posted from a session.

**2. The human writes their own intent.** Motivation — *why this matters, what
it is for* — is the maintainer's line, and an agent does not compose it in
someone else's voice. State what you did, what you measured, what you skipped.
Quoting the ask verbatim is honest and useful; paraphrasing it into a
first-person "I want this because…" is not. If the intent line is missing, leave
the slot and say it is missing rather than filling it.

**3. Do not hold the conversation.** Posting marked information on the
maintainer's behalf — a log, a benchmark, a diff — is fine. Replying to a
reviewer, conceding a point, agreeing a plan or closing an argument is not:
draft it in the session and let the human post it. An agent must not argue,
agree or decide as someone else.

Inside the tree there is nothing to mark: a commit an agent wrote carries
`Co-Authored-By: Claude …`, and that is the whole record. The rest of this file
is how the work itself is written.
...
- **Mark it, and fold the evidence away** — the note at the top, benchmark
  tables and logs in `<details>`. See *AI-assisted contributions*.
