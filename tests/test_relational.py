...
#: is positional rather than a join (compiler `_attached_bound`).
DENSE_BOUND_MODEL = {
...
#: order — an attach that read the source order as the label order would pair
#: every coordinate with another one's bound.
...
def _attached_for(model, data):
    ...
    real = compiler_module.PolarsCompiler._attached_bound
    seen = {}
    ...
    compiler_module.PolarsCompiler._attached_bound = spy
    try:
        lps.build(model, data).close()
    except LpspecError:
        pass  # the decision is recorded while the plan is built, before it runs
    finally:
        compiler_module.PolarsCompiler._attached_bound = real
    return seen
...
def test_a_bound_dense_over_the_product_is_attached_by_position_not_joined():
    ...
    frame is in label order by construction. Each row's row-major position is
    computable from its own coordinates, so the values scatter into that order
    and no join happens at all.
    ...
    assert _attached_for(FLAT_MODEL, data) == {'avail': True}
...
def test_the_positional_attach_places_by_coordinate_not_by_source_order():
    """The same numbers arriving shuffled must give the same model.

    A parameter's rows come in whatever order its source had. Attaching in that
    order would be right only for a file that happened to be written in label
    ...
    assert _attached_for(DENSE_BOUND_MODEL, shuffled) == {'avail': True}
...
def test_a_mask_or_a_sparse_bound_keeps_the_join():
    ...
    assert _attached_for(masked, {'avail': SHUFFLED_BOUND, 'cost': FLAT_COST}) == {'avail': False}

    holey = {'avail': SHUFFLED_BOUND.head(5), 'cost': FLAT_COST}
    assert _attached_for(DENSE_BOUND_MODEL, holey) == {'avail': False}
