...
class PolarsCompiler:
    ...
    def parameter_join(
        ...
        *maintain_order* is asked for only by the bounds, which become ``cols``
        and are read in order. It is far cheaper to ask the join for that order
        than to sort the same frame afterwards, so it is passed deliberately
    ...
    def _predicate(
        ...
        **A name the mask is certain of is joined rather than left-joined**,
        ...
        dropped from — worth having on the direct path, and nothing measurable
        behind a semi-join's key product.
    ...
    def _translate_fragment(self, p: TermFragment, s: plan.Translate, context: str) -> TermFragment:
        ...
        def travelled_presence() -> tuple[pl.LazyFrame | None, tuple[str, ...] | None]:
            ...
            coordinate product to name an edge, which measurably slows a wide
            ramp, a shape no case in `bench/` covers.
            """
