...
#: What a declared dtype accepts as bound values, in this lane's vocabulary.
#: The relational lane's ``ACCEPTED_VALUE_TYPES`` in polars types, pinned to it
#: and to the language's ``PARAMETER_DTYPES`` by ``tests/test_architecture.py``.
#: ``int`` serving ``float`` is the one widening, for the reason given there.
_ACCEPTED_KINDS: dict[str, str] = {'float': 'fiu', 'int': 'iu', 'bool': 'b', 'str': 'OUS'}

#: The declared dtype a numpy kind *is*, for the sentence both lanes share.
_DECLARED_KINDS: dict[str, str] = {
    'f': 'float',
    'i': 'int',
    'u': 'int',
    'b': 'bool',
...
def _check_values(name: str, values: pd.Series, dims: Sequence[str], declared: str) -> None:
    ...
    if kind not in _ACCEPTED_KINDS[declared]:
        raise DataError(wrong_value_dtype_message(name, declared, _DECLARED_KINDS.get(kind, str(values.dtype))))
