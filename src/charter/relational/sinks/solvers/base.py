...
for, so that a rebound model (:meth:`~charter.api.BoundModel.rebind`) has its new
...
class Solver(ABC):
    ...
    A driver never constructs one directly: :func:`~charter.relational.sinks.solvers.loaded`
    ...
    def __init__(
        ...
        #: :attr:`~charter.relational.sinks.tables.ModelTables.structure`, the
    ...
    #: (:func:`~charter.relational.sinks.sos.reformulated`).
    #:
    ...
    #: family acts on it (:func:`~charter.relational.sinks.solvers.ingestible`),
    ...
    def is_available(cls) -> bool:
        ...
        (:func:`~charter.relational.sinks.solvers.solver`).
    ...
    def warm(self, ws: WarmStart) -> None:
        ...
        Raises:
            CharterError: A warm start read from another solver, or whose
    ...
    def _takes(self, ws: WarmStart) -> None:
        ...
        Raises:
            CharterError: A start from another solver — statuses are each
    ...
    def _spans(self, quantity: str, values: pl.Series | None, expected: int) -> None:
        ...
        Raises:
            CharterError: A vector of any other length, which describes a
                different model.
        """
        if values is not None and len(values) != expected:
            raise CharterError(
