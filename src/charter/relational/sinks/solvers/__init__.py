...
:class:`~charter.relational.sinks.solvers.base.Solver` subclass named for it,
...
:mod:`~charter.relational.sinks.solvers.base` — the one module a member may read
besides ``tables.py``.
"""
...
from charter.relational.sinks import sos
from charter.relational.sinks.solvers.base import Solver
from charter.relational.sinks.solvers.gurobi import Gurobi
...
#: (:class:`~charter.relational.sinks.solvers.base.Solver`), and a one-shot
...
def solver(name: str) -> type[Solver]:
    ...
    (:attr:`~charter.relational.sinks.solvers.base.Solver.unavailable_message`),
    whether a solver ships or needs an extra being its own fact.

    Raises:
        CharterError: A name outside the closed set.
...
def ingestible(name: str, model: ModelTables) -> ModelTables:
    ...
    handed :func:`~charter.relational.sinks.sos.reformulated` tables, so no
...
def loaded(
    ...
    *held* is kept exactly when it is the named class holding a model that
    ...
    :attr:`~charter.relational.sinks.tables.ModelTables.structure`, same
