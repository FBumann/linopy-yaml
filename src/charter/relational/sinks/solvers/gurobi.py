...
The same hand-off as :mod:`~charter.relational.sinks.solvers.highs`, reading the
...
- **Nothing is batched.** The columns cannot be, since ``addMConstr`` writes
  ...
  :meth:`~charter.relational.sinks.tables.ModelTables.row_blocks`.
...
#: :attr:`~charter.relational.status.SolveStatus.is_readable` already refused
...
def build_gurobi(
    ...
    :func:`~charter.relational.sinks.solvers.highs.build_highs`'s seam, drawn
    ...
    :meth:`~charter.relational.sinks.tables.ModelTables.row_blocks` for why.
...
class Gurobi(Solver):
    ...
    :class:`~charter.relational.sinks.solvers.highs.Highs`'s twin, and the same
    ...
    - **Nothing pushes ``Sense``.** A row's comparison comes from the YAML and
      ...
      :attr:`~charter.relational.sinks.tables.ModelTables.structure` has already
    ...
    unavailable_message = 'The gurobi sink requires the [gurobi] extra (gurobipy, scipy): pip install "charter[gurobi]"'
