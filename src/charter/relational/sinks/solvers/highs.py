...
:mod:`~charter.relational.sinks.writers.lp_file`.

**Nothing textual crosses into numpy**: a row's ``'<='`` becomes a
:data:`~charter.relational.sinks.tables.SENSE_CODES` byte before it is read
here, the rule
:meth:`~charter.relational.sinks.tables.ModelTables.dense_columns` measured.
...
from charter.relational.sinks.solvers.base import SolveAnswer, Solver, WarmStart
from charter.relational.sinks.tables import SENSE_CODES, solver_vector
from charter.relational.status import SolveStatus

if TYPE_CHECKING:
    from collections.abc import Mapping

    from charter.relational.sinks.tables import ModelTables, RowVectors
...
#: through
#: :mod:`~charter.relational.chunking`, which asks a caller to state what one
...
def build_highs(
    ...
    :mod:`~charter.relational.chunking`; the parameter stays so tests can force
    ragged chunks.
    """
    import highspy
    import numpy as np
...
class Highs(Solver):
    ...
    :meth:`~charter.relational.sinks.solvers.base.Solver.warm`.
    ...
    **Values are re-pushed, never diffed** — the previous model is *gone* by
    ...
    requires = ('highspy',)
    unavailable_message = 'highspy ships with charter, so a build without it is broken rather than missing an extra'
...
def _loaded(h: Any, status: Any, what: str) -> None:
    ...
    Raises:
        CharterError: If the batch was refused.
    """
    import highspy
...
def _took(status: Any, what: str) -> None:
    ...
    Raises:
        CharterError: If the hint was refused.
    """
    import highspy
