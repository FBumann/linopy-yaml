...
(:mod:`charter.relational.engines.polars.binding`), what a query over it looks like
(:mod:`charter.relational.engines.polars.compiler`), which coordinate gets which solver index
(:mod:`charter.relational.engines.polars.labels`). The lane is described in
...
which is what :class:`~charter.relational.engines.polars.binding.BoundSources` says.
"""
...
from charter.relational import plan, sinks
from charter.relational.engines.polars import labels
from charter.relational.engines.polars.binding import BoundSources, bind
from charter.relational.engines.polars.compiler import PolarsCompiler, Presence, TermFragment
from charter.relational.result import Diagnostics, Result
from charter.relational.sinks.tables import SENSE
...
#: :data:`~charter.relational.plan.VariableType` and not reaching here fails
...
class _Block:
    ...
    :func:`~charter.relational.engines.polars.labels.frame` numbers a
...
class PolarsEngine:
    ...
    def build(
        ...
        **``rows`` leaves in row order too**, which a solver reads as its own
        index — so :meth:`~charter.relational.sinks.tables.ModelTables.dense_rows`
        takes the frame's own vectors instead of scattering by label on every
        solve, and :attr:`~charter.relational.sinks.tables.ModelTables.structure`
    ...
    def _build_sos(self, s: plan.SosDeclaration, v: plan.VariableDeclaration) -> pl.DataFrame:
        ...
        **A set and a weight are the two halves of a coordinate's position in
        ...
        (:func:`~charter.relational.engines.polars.labels.row_major`) numbering
        ...
        **A position is the label where the variable dropped nothing.** Labels
        ...
        same split :func:`~charter.relational.engines.polars.labels.frame`
        ...
        **The stream leaves grouped by set and ascending in weight**, which is
        ...
        through
        :func:`~charter.relational.engines.polars.labels.in_position_order`,
    ...
    def solve(
        ...
        *,
        ...
        :func:`~charter.relational.sinks.solvers.loaded`'s decision and not this
        ...
        **What the solver is handed may be wider than what was built**, and
        ...
        (:func:`~charter.relational.sinks.solvers.ingestible`): a sink with no
        ...
        Args:
            ...
            batch_rows: The hand-off budget in elements, defaulting to the
                sink's own
                (:data:`~charter.relational.sinks.solvers.highs.HANDOFF_BUDGET`).
            ...
            warm: Whether this solve may start from what the session holds.
                ...
                a member squirrels away. :attr:`~charter.relational.result.Diagnostics.loads`
...
def _row_starts(ordered: pl.DataFrame, row_count: int) -> Any:
    ...
    :meth:`~charter.relational.sinks.tables.ModelTables.matrix_block` to spell
    the labels back out.
