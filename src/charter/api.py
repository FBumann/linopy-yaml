...
compatibility/oracle layer (``import charter.linopy``).
...
from charter.errors import CharterWarning, DataError
from charter.language.validation import load_model
from charter.lowering import advice, expression_thunks, lower_program
from charter.relational.engines.polars.engine import PolarsEngine
from charter.relational.sinks import solver, writer
...
def check(model: str | Path | dict[str, Any] | Model) -> Model:
    ...
    Warns:
        CharterWarning: Advice short of an error — a declared dimension nothing
    ...
    for note in advice(program):
        warnings.warn(note, CharterWarning, stacklevel=2)
    return schema
...
class BoundModel:
    ...
    def rebind(
        ...
        *,
        ...
        :attr:`~charter.relational.result.Diagnostics.loads` says which ran.
        ...
        is dropped or :meth:`~charter.relational.result.Result.close` is called.
    ...
    def solve(
        ...
        *,
        ...
        :attr:`~charter.relational.result.Result.started`.
        ...
        Raises:
            CharterError: A solver name nothing serves, or one this
...
def solve(
    ...
    **build_kwargs: Any,
    ...
    Raises:
        CharterError: A solver name nothing serves — checked before the build.
    """
    solver(solver_name)
