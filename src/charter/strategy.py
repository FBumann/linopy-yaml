...
:mod:`charter.api`, built from the public verbs, and no lane learns a new word.
...
from charter.api import build, check
from charter.api import solve as _solve
from charter.errors import CharterError, DataError
from charter.relational.frames import as_frame
from charter.relational.result import tidy_to_dataarray, tidy_to_dataset, tidy_to_pandas
...
class _CarryRule:
    ...
    def resolved(cls, schema: Model, parameter: str, variable: str, index: int | None) -> _CarryRule:
        ...
        **Nothing here reads data**, which is why the plan resolves before the
        ...
        if parameter not in schema.parameters:
            raise CharterError(f'carry writes parameter {parameter!r}, which the model does not declare')
        if variable not in schema.variables:
            raise CharterError(f'carry reads variable {variable!r}, which the model does not declare')
    ...
    def value_from(self, frames: Mapping[str, pl.DataFrame], parameter: str, key: Any) -> pl.DataFrame:
        ...
        Raises:
            CharterError: ``index`` names a coordinate the slice does not have
...
class _OriginalIndex:
    ...
    **One-way, and that is why it holds only the owned coordinates.** It is the
    ...
    def restore(self, frame: pl.DataFrame, key_name: str) -> pl.DataFrame:
        ...
        Raises:
            CharterError: *frame* has no ``local`` column — the quantity was
...
class Runs:
    ...
    :class:`~charter.relational.result.Result`'s readers one dimension wider —
    ...
    def _read(
        ...
        *absent* is a reason the fold already knows, which beats one derived
        ...
        """
        if name not in held:
            raise CharterError(absent or _nothing_to_read(kind, name, held, self.meta))
    ...
    def primal(self, name: str, *, original_index: bool = False) -> pl.DataFrame:
        ...
        Raises:
            CharterError: No slice of the sweep produced *name*.
        """
    ...
    def dual(self, name: str, *, original_index: bool = False) -> pl.DataFrame:
        ...
        Raises:
            CharterError: No slice produced duals for *name* — the message says
    ...
    def expression(self, name: str, *, original_index: bool = False) -> pl.DataFrame:
        ...
        Raises:
            CharterError: No slice produced *name* — an evaluation that failed
    ...
    def to_dataset(self, *names: str) -> xr.Dataset:
        ...
        Raises:
            CharterError: The sweep holds no variable values at all.
        """
        wanted = names or tuple(sorted(self._primals))
        if not wanted:
            raise CharterError(_nothing_to_read('variable', 'anything', self._primals, self.meta))
...
def solve_over(
    ...
    **A process pool must not use the ``fork`` start method.** polars' thread
    ...
    Raises:
        CharterError: A carry together with an executor — a carried value makes
...
def _serially(
    ...
    what :meth:`~charter.api.BoundModel.rebind` is: the YAML is parsed once, the
...
def _answers(result: Any, model: Any) -> _Answer:
    ...
    :meth:`~charter.relational.result.Result.expression` defers: its deferred
