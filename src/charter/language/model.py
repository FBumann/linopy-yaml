...
from charter.errors import did_you_mean, schema_error
...
class CoordinateSpec(_StrictBlock):
    ...
    (:func:`charter.language.resolution.resolve`).
    """
...
class Model(_StrictBlock):
    ...
    *says*, ``plan.Program`` what it lowers to, an engine what a build holds.
    ...
    :class:`~charter.errors.LanguageError` on a model the language refuses.
    ...
    #: read, keyed — by :func:`~charter.language.piecewise.expand_piecewise`,
    ...
    def _check_version(cls, v: int) -> int:
        ...
        than ``charter.__version__``: a language module may not reach forward to
        the package that consumes its AST.
        """
        if v in SUPPORTED_VERSIONS:
            return v
        try:
            installed = metadata.version('charter')
        ...
        msg = (
            f'model declares version {v}, and charter {installed} understands [{supported}]. '
            f'Upgrade charter, or write the version this file actually targets.'
        )
        raise ValueError(msg)
    ...
    def _validate_expressions(self, info: ValidationInfo) -> Model:
        ...
        already on the model ``charter.linopy.extend`` puts it on. It travels
        into expansion too, a link being able to name one.
        """
        from charter.language.piecewise import expand_piecewise
        from charter.language.validation import validate_expressions
