"""``python -m charter <format> model.yaml`` — the typeset shell front.
...
:data:`charter.typeset.FORMATS`, so a new format arrives with its verb written.
...
def parser() -> argparse.ArgumentParser:
    ...
    prose.
    """
    front = argparse.ArgumentParser(prog='python -m charter')
    verbs = front.add_subparsers(dest='verb', required=True)

    for name in FORMATS:
        verb = verbs.add_parser(name, help=f'render a model as {name}')
        verb.add_argument('model', help='path to a charter YAML model')
