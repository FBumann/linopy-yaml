...
def index(stamped: dict) -> str:
    rows = '\n'.join(
        ...
        f' {_cell_prices(stamped[s]["parity"])} | {_cell_blocks(stamped[s]["structural"])} |'
        f' {sum(1 for c in stamped[s]["parity"]["built_rows"].values() if c)} |'
        for s in stems()
    )
    return (
        '# The PyPSA ladder\n\n'
        ...
        " same network, and compared with PyPSA three ways. The `PyPSA parity` workflow regenerates every page's"
