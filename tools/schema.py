"""The checked-in JSON Schema for the YAML surface.
    ...
    uv run python -m tools.schema   # rewrite schema/lpspec.schema.json
...
from lpspec import Model

PATH = Path(__file__).resolve().parent.parent / 'schema' / 'lpspec.schema.json'
...
def _canonical(node: Any) -> Any:
    ...
    render must not depend on which generated it. Three rewrites:
    ...
    if not isinstance(node, dict):
        return node
    out = {k: _canonical(v) for k, v in node.items() if not (isinstance(v, float) and math.isinf(v))}
