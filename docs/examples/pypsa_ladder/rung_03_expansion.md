    ...
    expressions:
      ...
      tech_capacity_expansion:
        description: what a `tech_capacity_expansion_limit` row totals — the chosen build of the row's carrier-and-bus
          set
        expression: sum(Generator_p_nom_ext * Generator_tech_capacity_weight, over=generator) + sum(Link_p_nom_ext
          * Link_tech_capacity_weight, over=link) + sum(StorageUnit_p_nom_ext * StorageUnit_tech_capacity_weight,
          over=storage_unit) + sum(Store_e_nom_ext * Store_tech_capacity_weight, over=store)
    objective:
      sense: minimize
