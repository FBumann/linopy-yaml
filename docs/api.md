...
| a carry is checked before anything solves | the dims come from the YAML, so a carry that cannot line up — collapsing two dimensions at once, a parameter over more than the variable is, an index where the sides already match — is a call-time error, not a surprise after slice one |
