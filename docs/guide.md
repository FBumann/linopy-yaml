...
| | |
|---|---|
| **A dimension is an axis, and its coordinates usually come from the data.** One master set per dimension, resolved before anything binds, so two tables disagreeing about which snapshots exist is a load-time error rather than a truncated model. | [dimensions](https://math-spec.readthedocs.io/en/latest/reference/language/dimensions/) · [dispatch](examples/dispatch.md) |
...
[the data contract](reference/data.md) is what binds, what is refused, and
...
| | |
|---|---|
| [Preparing the data](examples/data.md) · [the contract](reference/data.md) | files to frames, and what binding refuses |
