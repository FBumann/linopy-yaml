...
The second engine settled two things about where such a bound would have to go,
and neither is where it was expected. **An engine-side limit does not reach
peak** — duckdb has `SET memory_limit`, and a 40M-column build capped at 500 MB
peaks at the same figure as an uncapped one, its own working set having been
under the cap all along. **Nor would a bound over `ModelTables`**: since the
matrix went CSR the four frames are about a third of peak on either engine, and
the build transient is the larger term. Numbers, method and provenance:
