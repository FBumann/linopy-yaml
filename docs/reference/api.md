...
is [the language](https://energy-models.github.io/math-spec/reference/language); this page is what loads, checks, builds,
...
| `math_spec.to_latex` / `to_typst` / `to_markdown` | the math as a document — [typeset](https://energy-models.github.io/math-spec/reference/typeset) |
...
([errors](https://energy-models.github.io/math-spec/reference/language/errors#which-error-you-get)).
...
| **a solver can make a model mixed-integer** | an [`sos:`](https://energy-models.github.io/math-spec/reference/language/piecewise#sos) set reaches a solver with no SOS concept as binaries, so an otherwise continuous model solved on `highs` has no duals and says so. On `gurobi` and `xpress`, which branch on the set itself, it keeps them |
...
| `omissions` | rows a constraint declared but did not build ([absence](https://energy-models.github.io/math-spec/reference/language/absence#a-row-with-no-variable-terms-is-not-built)) |
