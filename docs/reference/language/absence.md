...
| a [`piecewise:`](piecewise.md#piecewise) curve pinned off where its gate does not exist | `absence: zero` on the gate variable — without it the block is refused, an absent gate having deleted the convexity row rather than the curve |
